...
class ProstT5Model(BaseEmbeddingModel):
    ...
    def __init__(self, model_name: str = None, device: torch.device = None, **kwargs):
        ...
        self.use_half_precision = kwargs.get('use_half_precision', True)  # Default to half precision for GPU
        self.sequence_type = kwargs.get('sequence_type', 'amino_acid')  # 'amino_acid' or '3di'
        self.layer_idx = kwargs.get('layer_idx', -1)  # Layer to extract embeddings from (-1 for last layer)
        self.return_all_layers = kwargs.get('return_all_layers', False)  # Return embeddings from all layers
