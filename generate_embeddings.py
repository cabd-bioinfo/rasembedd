...
def load_sequences_from_tsv(
    tsv_file: str, id_column: str = "uniprot_id", seq_column: str = "sequence"
) -> Dict[str, str]:
...
def save_embeddings(
    embeddings: Dict[str, np.ndarray], output_file: str, format: str = "pickle"
) -> None:
    ...
    try:
        ...
        for attr_name in dir(module):
            ...
            if (
                ...
                and issubclass(attr, BaseEmbeddingModel)
                and attr != BaseEmbeddingModel
            ):
                return attr
        ...
        raise ValueError(
            f"Model type '{model_type}' not found. Available models: {available_models}. Error: {str(e)}"
        )
...
def main():
    ...
    # Input arguments
    parser.add_argument("--input", required=True, help="Path to input file (FASTA or TSV)")
    parser.add_argument(
        "--input_type",
        choices=["fasta", "tsv"],
        default="tsv",
    ...
    parser.add_argument(
        "--seq_column", default="sequence", help="Column name for sequences (TSV only)"
    )
    ...
    parser.add_argument("--model_name", help="Specific model name/path (model-specific)")
    parser.add_argument("--device", default="auto", help="Device to use (auto, cuda, or cpu)")

    # Output arguments
    parser.add_argument("--output", required=True, help="Path to output file for embeddings")
    parser.add_argument(
        "--format",
        choices=["pickle", "npz", "hdf5", "h5"],
        default="pickle",
