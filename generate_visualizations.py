...
def load_embeddings(embeddings_path: str) -> Dict[str, np.ndarray]:
    ...
    if path.suffix == '.pkl':
        ...
        raise ValueError(f"Unsupported file format: {path.suffix}. Supported formats: .pkl, .npz, .h5, .hdf5")
...
def plot_heatmap(distances: np.ndarray, labels: List[str], color_labels: List[int], 
                output_file: str, color_column: str, class_names: List[str],
                figsize: Tuple[int, int] = (10, 10), palette_name: str = 'glasbey',
                show_dendrograms: bool = True, cluster_method: str = 'average') -> None:
    ...
    # Create legend
    handles = [plt.Line2D([0], [0], marker='o', color=color, markersize=8, 
...
def compute_projection(embeddings: np.ndarray, method: str, random_seed: int = 42, **kwargs) -> np.ndarray:
...
def plot_projection(projection: np.ndarray, labels: List[int], method: str, 
                   class_names: List[str], output_file: str, 
                   figsize: Tuple[int, int] = (12, 9), palette_name: str = 'glasbey',
                   random_seed: int = 42, protein_ids: Optional[List[str]] = None, 
                   reducer_params: dict = None, **kwargs) -> None:
        ...
        for species in unique_species:
            ...
            if species_indices:
                ...
                plt.scatter(
                    ...
                    edgecolors='black',
                    linewidth=0.1,
                    label=f'{species}' if len(unique_species) <= 10 else None  # Only show species labels if not too many
                )
    else:
    ...
    if show_labels and protein_ids is not None:
        ...
        if n_points <= max_labels:
            ...
            for i, (x, y, pid) in enumerate(zip(projection[:, 0], projection[:, 1], protein_ids)):
                plt.annotate(pid, (x, y), xytext=(5, 5), textcoords='offset points', 
                           fontsize=label_fontsize, alpha=0.8)
        else:
            # Show only a subset of labels to avoid overcrowding
            print(f"Warning: Too many points ({n_points}) to label all. Showing {max_labels} labels.")
    ...
    # Class color legend
    class_handles = [plt.Line2D([0], [0], marker='o', color=color, markersize=10, 
                               linestyle='', label=class_name)
    ...
    if show_species and len(unique_species) > 1:
        species_handles = [plt.Line2D([0], [0], marker=marker, color='gray', markersize=10,
                                    linestyle='', label=species, markerfacecolor='gray')
        ...
        if len(class_names) <= 8 and len(unique_species) <= 8:
            ...
            class_legend = plt.legend(handles=class_handles, 
                                    loc='upper left', bbox_to_anchor=(1.05, 1),
                                    frameon=False, fontsize=9)
            plt.gca().add_artist(class_legend)  # Keep first legend when adding second
            
            species_legend = plt.legend(handles=species_handles, 
        ...
        if len(class_names) <= 15:
            class_legend = plt.legend(handles=class_handles, loc='best', 
                                    frameon=False, fontsize=9)
        else:
            class_legend = plt.legend(handles=class_handles, 
                                    bbox_to_anchor=(1.05, 1), loc='upper left', 
                                    frameon=False, 
                                    fontsize=8, ncol=max(1, len(class_names) // 15))
...
def validate_inputs(df: pd.DataFrame, embeddings: Dict[str, np.ndarray], 
                   color_column: str, id_column: str = 'uniprot_id') -> Tuple[pd.DataFrame, np.ndarray]:
    """Validate and align inputs."""
    
    # Check if ID column exists
    if id_column not in df.columns:
        raise ValueError(f"ID column '{id_column}' not found in metadata. Available columns: {list(df.columns)}")
    
    # Check if color column exists
    if color_column not in df.columns:
        raise ValueError(f"Color column '{color_column}' not found in metadata. Available columns: {list(df.columns)}")
...
def get_output_filename(base_name: str, method: str, color_column: str, 
                       embeddings_file: str, output_format: str = 'pdf') -> str:
    ...
    if '_' in base_name:
        ...
        if possible_prefix and possible_prefix.lower() not in [method_lower, 'heatmap', 'projection', 'umap', 'tsne', 'pca']:
    ...
    if prefix:
        ...
        if '/' in prefix:
            ...
            filename = f"{dir_part}/{prefix_only}_{method_part}_{color_column}_{emb_name}.{output_format}"
        else:
...
Examples:
  ...
  python generate_visualizations_improved.py data/metadata.tsv -e model1.pkl \\
    ...
    parser.add_argument('-e', '--embeddings', 
                       help='Path to embeddings file (.pkl, .npz, .h5, .hdf5)')
    
    # Visualization options
    parser.add_argument('-c', '--color_column', default='Family.name',
                       help='Column name for coloring (default: Family.name)')
    parser.add_argument('--color_columns', nargs='+',
    ...
    parser.add_argument('-m', '--distance_metric', default='cosine', 
    ...
    # Appearance options
    parser.add_argument('--palette_name', default='glasbey',
                       choices=['glasbey', 'tab10', 'Set3', 'husl'],
                       help='Color palette (default: glasbey)')
    parser.add_argument('--figsize', nargs=2, type=float, default=[11.69, 8.27],
                       help='Figure size width height in inches (default: 11.69 8.27, A4 landscape)')
    parser.add_argument('--output_format', default='pdf', choices=['pdf', 'png', 'svg'],
    ...
    parser.add_argument('--skip_projection', action='store_true',
                       help='Skip projection generation')
    ...
    try:
        ...
        if args.embedding_files:
            ...
            raise ValueError("No embedding files specified")  # This shouldn't happen due to validation above
        ...
        for color_col in color_columns:
            if color_col not in df.columns:
                raise ValueError(f"Color column '{color_col}' not found in metadata. Available columns: {list(df.columns)}")
        
        print(f"Processing {len(embedding_files)} embedding file(s) with {len(color_columns)} color column(s)")
        
        # Process each embedding file
        for emb_idx, embeddings_file in enumerate(embedding_files):
            print(f"\n{'='*60}")
            print(f"Processing embedding file {emb_idx+1}/{len(embedding_files)}: {embeddings_file}")
            print(f"{'='*60}")
            ...
            for color_idx, color_column in enumerate(color_columns):
                ...
                df_filtered, embeddings_array = validate_inputs(df, embeddings, color_column, args.id_column)
                ...
                if not args.skip_heatmap:
                    ...
                    heatmap_output = get_output_filename(
                        ...
                        "heatmap", color_column, embeddings_file, args.output_format
                    )
                    plot_heatmap(
                        distances, df_filtered[args.id_column].tolist(), color_labels,
                        heatmap_output, color_column, class_names,
                        figsize=tuple(args.figsize), palette_name=args.palette_name
                    )
                ...
                if not args.skip_projection:
                    ...
                    if color_idx == 0:  # First color column - compute projections
                        ...
                        for method in methods:
                            projection, reducer_params = compute_projection(
                                embeddings_array, method, args.random_seed,
                    ...
                    for method in methods:
                        ...
                        projection_output = get_output_filename(
                            ...
                            method, color_column, embeddings_file, args.output_format
                        )
                        plot_projection(
                            projection, color_labels, method, class_names,
                            projection_output, figsize=tuple(args.figsize), 
                            palette_name=args.palette_name, random_seed=args.random_seed,
