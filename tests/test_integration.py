...
class TestFullPipeline:
    ...
    def test_generate_visualizations_script(self, mock_subprocess, temp_dir):
        ...
        np.random.seed(42)
        mock_embeddings = {
            seq_id: np.random.randn(128).astype(np.float32) for seq_id in SAMPLE_SEQUENCES.keys()
        }
    ...
    def test_data_flow_consistency(self, temp_dir):
        ...
        np.random.seed(42)
        embeddings = {
            seq_id: np.random.randn(512).astype(np.float32) for seq_id in sequences.keys()
        }
    ...
    def test_error_propagation(self, temp_dir):
        ...
        sequences = SAMPLE_SEQUENCES.copy()
        embeddings = {
            seq_id: np.random.randn(128).astype(np.float32)
            for seq_id in list(sequences.keys())[:-1]
    ...
    def test_scalability_simulation(self, temp_dir):
        ...
        for i in range(n_sequences):
            ...
            aa_sequence = "".join(
                np.random.choice(list("ACDEFGHIKLMNPQRSTVWY"), size=np.random.randint(50, 200))
            )
    ...
    def test_version_compatibility(self):
        ...
        packages_to_check = ["numpy", "pandas", "matplotlib", "scikit-learn"]
